    ...
    <div class="col-md-4">
        ...
        <div class="form-group">
            <div class="custom-control custom-checkbox ">
                <input type="radio" id="PASSED" name="result" class="custom-control-input" value="PASSED" @if(isset($answer['result']) && $answer['result']=='PASSED') checked @endif @if(Auth::user()->privilege->type == 'ADMIN') disabled @endif>
                <label class="custom-control-label" for="PASSED">Dapat Disarankan</label>
            </div>
            <div class="custom-control custom-checkbox ">
                <input type="radio" id="RECONSIDERED" name="result" class="custom-control-input" value="RECONSIDERED" @if(isset($answer['result']) && $answer['result']=='RECONSIDERED') checked @endif @if(Auth::user()->privilege->type == 'ADMIN') disabled @endif>
                <label class="custom-control-label" for="RECONSIDERED">Dipertimbangkan</label>
            </div>
            <div class="custom-control custom-checkbox ">
                <input type="radio" id="REJECTED" name="result" class="custom-control-input" value="REJECTED" @if(isset($answer['result']) && $answer['result']=='REJECTED') checked @endif @if(Auth::user()->privilege->type == 'ADMIN') disabled @endif>
                ...
                <tbody>
                    @for($i = 0; $i < 10; $i++)
                        <tr>
                            ...
                            <td><input class="form-control" type="text" name="score[]" placeholder="Skor" value="@if(isset($answer['competencies'][$i])) {{$answer['competencies'][$i]->score}} @endif" @if(Auth::user()->privilege->type == 'ADMIN') disabled @endif></td>
